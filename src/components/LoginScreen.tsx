...
export default function LoginScreen({ sessionUser, onLogin, onLogout }: LoginScreenProps) {
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const password = loginPassword === "••••••••" ? "admin123" : loginPassword;
      
      let userCredential;
      try {
        userCredential = await signInWithEmailAndPassword(auth, loginEmail.trim(), password);
      } catch (authError: any) {
        // Auto-create default demo account if it doesn't exist
        if (loginEmail.trim() === "admin@mayfieldrepairs.com.au" && (authError.code === "auth/user-not-found" || authError.code === "auth/invalid-credential")) {
          userCredential = await createUserWithEmailAndPassword(auth, loginEmail.trim(), "admin123");
      ...
      let userRole: "Admin" | "Manager" | "Agent" = loginRole;
      ...
      setIsAuthenticating(false);
      runHandshakeAnimation("credentials", loginEmail, userRole);
    } catch (err: any) {
      setIsAuthenticating(false);
      ...
      if (err.code === "auth/wrong-password") {
        ...
        friendlyMessage = "User account not found. Please sign up to create a new portfolio workspace.";
  ...
  const handleCredentialsSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save user details to Firestore via backend API
      await fetch("/api/users", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
